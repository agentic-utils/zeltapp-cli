...
func (s *fileStore) SetPassword(email, password string) error {
	...
	// inline value reads the password from a prompt; we redirect that prompt
	// to stdin and provide the bytes ourselves. `-U` updates if the entry
	// already exists.
	cmd := exec.Command(s.keychainCmd, "add-generic-password",
		"-a", email, "-s", keychainService, "-w", "-U")
