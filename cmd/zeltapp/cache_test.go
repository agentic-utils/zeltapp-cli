...
func TestCache_RoundtripFile(t *testing.T) {
	...
	if err := cacheSet("test", path, body, 5*time.Minute); err != nil {
		t.Fatal(err)
	}
	got, ok := cacheGet("test", path)
	if !ok {
...
func TestCache_ExpiresOnTTL(t *testing.T) {
	...
	if err := cacheSet("test", path, body, -1*time.Minute); err != nil {
		t.Fatal(err)
	}
	if _, ok := cacheGet("test", path); ok {
...
func TestCache_ClearWipesEverything(t *testing.T) {
	...
	_ = cacheSet("test", "/apiv2/users/cache", json.RawMessage(`{}`), 5*time.Minute)
...
func TestCmd_CacheList(t *testing.T) {
	...
	_ = srv
	if err := runCmd(t, "config", "cache", "list"); err != nil {
		t.Fatal(err)
	}
...
func TestCmd_CacheListEmpty(t *testing.T) {
	...
	srv, _, _, buf := withTestEnv(t)
	_ = srv
	if err := runCmdHuman(t, "config", "cache", "list"); err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(buf.String(), "(empty)") {
		t.Errorf("expected (empty) marker for empty cache, got: %s", buf.String())
	}
}
...
func TestCmd_CacheClear(t *testing.T) {
	...
	_ = srv
	if err := runCmd(t, "config", "cache", "clear"); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(dir); !os.IsNotExist(err) {
